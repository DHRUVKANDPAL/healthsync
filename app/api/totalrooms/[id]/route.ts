...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const roomData = {
      singleRoom: roomCounts.find(room => room.typeof === "Single Room")?._count.typeof || 0,
      ICU: roomCounts.find(room => room.typeof === "ICU")?._count.typeof || 0,
      GeneralWard: roomCounts.find(room => room.typeof === "General Ward")?._count.typeof || 0,
      SharedRoom: roomCounts.find(room => room.typeof === "Shared Room")?._count.typeof || 0,
    };

    // Calculate the total rooms
    const total = roomData.singleRoom + roomData.ICU + roomData.GeneralWard + roomData.SharedRoom;

    // Return a success response with room data
    return createResponse({ total: { ...roomData, total }, success: true }, 200);
